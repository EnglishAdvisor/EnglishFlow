/* FIRST OIL — battle.js
   Execução de tarefas e controle de crises: perguntas com timer, sequência de
   acertos (uptime), críticos por agilidade, itens do almoxarifado e crises com
   fases e mecânicas próprias. Também roda os modos de contagem fixa (O.S. e DDS). */
(function (root) {
  ...
  B.start = function (cfg) {
    ...
    G.AU.music(anyCrisis() ? 'boss' : 'battle', cfg.regionIdx || 0);
  ...
  function anyCrisis() { return S.enemies.some(function (e) { return !!e.crisis; }); }
  ...
  function nextEnemy() {
    ...
    G.$('#b-ename').innerHTML = '';
    const nm = G.el('span', spec.crisis ? 'boss-name' : '', '');
    nm.textContent = (spec.nc ? '⚠ ' : '') + spec.name + (spec.elite ? ' · CRÍTICA' : '');
    ...
    G.$('#b-ehplabel').textContent = fixedMode() ? '' : (spec.crisis ? 'SEVERIDADE DA CRISE' : 'TRABALHO RESTANTE');
    drawEnemyHp();
    if (spec.crisis) { G.AU.sfx('boss'); G.FX.shake(true); }
    else if (spec.nc) G.AU.sfx('ghost');
  ...
  function drawEnemyHp() {
    ...
    const frac = G.clamp(S.enemy.hp / S.enemy.max, 0, 1);
    G.$('#b-ehp').style.width = (frac * 100) + '%';
    if (S.mon && S.mon.setSeverity) S.mon.setSeverity(frac);
  }
  ...
  function showDialogue(name, lines, done) {
    ...
    function next() {
      ...
      const line = lines[i].replace(/\{name\}/g, G.state.name || 'colega');
      i++;
  ...
  function ask() {
    ...
    if (fixedMode()) {
      ...
      if (S.ch.isReview) G.toast('⚠ NC reaberta: um termo que você errou voltou para verificação.', { once: 'review' });
    }
    S.qi++;
    ...
    S.hintUsed = false;
    renderItems();
    // mecânica da auditora: embaralha as alternativas no meio da pergunta
    if (S.enemy.spec.mech === 'audit' && S.phase >= 1 && S.ch.ui === 'choice') {
  ...
  function renderChallenge(ch) {
    ...
    // trecho de leitura técnica
    if (ch.passage) {
      const ps = G.el('div', 'passage', '');
      ps.textContent = ch.passage;
      area.appendChild(ps);
      if (ch.question) {
        const q = G.el('div', 'passage-q', '');
    ...
    if (ch.listenOnly && ch.tts) {
      const play = G.el('button', 'btn listen-btn', '🔊 Reproduzir transmissão');
    ...
    if (ch.ui === 'choice') {
      ...
      inp.placeholder = ch.numeric ? 'digite o número...' : 'digite em inglês...';
      if (ch.numeric) inp.inputMode = 'numeric';
      inp.autocapitalize = 'off'; inp.autocomplete = 'off'; inp.spellcheck = false;
      inp.onkeydown = function (e) { if (e.key === 'Enter') submit(); };
      const btn = G.el('button', 'btn primary', 'Registrar ▸');
      function submit() {
        if (!S || !S.waiting) return;
      ...
      function refresh() {
        built.innerHTML = '';
        if (!placed.length) built.appendChild(G.el('span', 'tile-hint', 'toque nos itens na ordem correta'));
      ...
      const hear = G.el('button', 'btn listen-btn', '🔊 Ouvir pronúncia padrão');
      hear.onclick = function () { speakTts(ch); };
      const mic = G.el('button', 'btn mic-btn', '🎤 Apertar para transmitir');
      const status = G.el('div', 'mic-status muted', '');
      let tries = 0;
      mic.onclick = async function () {
        if (!S || !S.waiting) return;
        mic.disabled = true; mic.textContent = '🎙️ Na escuta...';
        status.textContent = 'Transmita: "' + ch.speakTarget + '"';
        const r = await G.AU.listen({ timeout: 6000 });
        if (!S || !S.waiting) return;
        mic.disabled = false; mic.textContent = '🎤 Apertar para transmitir';
        if (r.ok) {
          ...
          tries++;
          status.textContent = 'Copiei "' + r.alts[0].t + '"... repita a transmissão (' + tries + '/3).';
          G.AU.sfx('bad');
          if (tries >= 3) resolve({ ok: false, close: false, input: r.alts[0].t });
        } else {
          status.textContent = r.err === 'not-allowed'
            ? 'Microfone bloqueado — libere o acesso no navegador.'
            : 'Sem cópia... transmita de novo.';
        }
      };
  ...
  function speakTts(ch) {
    ...
    if (S && S.enemy.spec.mech === 'muster' && S.phase >= 1) rate = 1.18;
    if (S && S.enemy.spec.mech === 'kick' && S.phase >= 1) rate = 1.1;
  ...
  function resolve(res) {
    ...
    if (res.ok) {
      ...
      if (S.enemy.spec.nc) dmg *= 1.25;
      dmg = Math.round(dmg);
      S.enemy.hp = Math.max(0, S.enemy.hp - dmg);
      drawEnemyHp();
      const c = enemyCenter();
      G.FX.burst(c.x, c.y, { n: crit ? 30 : 16, colors: crit ? ['#fff', '#ffc400', '#ff7a1a'] : ['#ffc400', '#57c7ff', '#fff'], speed: crit ? 4.5 : 3 });
      G.FX.floatText(c.x, c.y - 30, '+' + dmg + '%' + (crit ? ' ⚡' : ''), crit ? 'crit' : 'dmg');
      S.mon.hurt();
      G.AU.sfx(crit ? 'crit' : 'hit');
      if (S.combo >= 2) G.AU.sfx('combo', S.combo);
      G.vibrate(20);
      if (S.combo === 3) G.toast('📈 Uptime x3! Acertos em sequência aumentam seu avanço.', { once: 'combo' });
    } else {
      S.combo = 0;
      S.mon.attack();
      ...
      setTimeout(function () {
        if (!S) return;
        G.FX.shake(!!spec.crisis);
        G.FX.vignette();
        G.AU.sfx('hurt');
        G.vibrate([40, 40, 40]);
        if (!fixedMode()) {
          let edmg = 9 + (S.cfg.regionIdx || 0) * 3 + (spec.crisis ? 4 : 0);
  ...
  function showFeedback(res, fast) {
    ...
    let head;
    if (res.ok && res.close) head = '📝 QUASE — aceito com erro de digitação';
    else if (res.ok && fast) head = '⚡ CORRETO — EXECUÇÃO EXEMPLAR!';
    else if (res.ok) head = '✔ CORRETO';
    else if (res.timeout) head = '⏰ TEMPO EXCEDIDO';
    else if (res.trap) head = '🚩 ' + (ch.trapNote || 'PEGADINHA CLÁSSICA!');
    else head = '✘ INCORRETO';
    fb.innerHTML = '';
    ...
    if (spoken && ch.kind !== 'read') setTimeout(function () { G.AU.speak(spoken, { rate: 0.95 }); }, 350);
  ...
  function afterFeedback() {
    ...
    if (!fixedMode() && S.enemy.spec.crisis) {
  ...
  function enterPhase(p, line) {
    ...
    G.FX.shake(true);
    G.AU.sfx('boss');
    if (S.enemy.spec.mech === 'kick') {
      S.timeMult = p === 1 ? 0.85 : 0.72;
      G.toast('📟 Pressão subindo — o tempo de resposta encurtou!');
    }
  ...
  function onEnemyDead() {
    const spec = S.enemy.spec;
    const c = enemyCenter();
    G.FX.burst(c.x, c.y, { n: 40, colors: ['#ffc400', '#fff', '#57c7ff', '#6bffb0'], speed: 5 });
    G.AU.sfx(spec.crisis ? 'fanfare' : 'coin');
    S.mon.die(function () {
      if (!S) return;
      if (S.ei < S.enemies.length - 1) {
        S.ei++;
        G.toast('📋 Próxima tarefa da O.S. interna...');
        setTimeout(function () { if (S) nextEnemy(); }, 500);
      } else {
        victory();
      }
    });
    if (spec.defeatLine) S.defeatLine = spec.defeatLine;
  }
  ...
  function victory() {
    ...
    S.enemies.forEach(function (spec) {
      if (spec.crisis) { coins += 45 + r * 12; xp += 60 + r * 18; G.state.stats.bosses++; }
      else if (spec.nc) { coins += 14 + r * 3; xp += 20 + r * 4; }
    ...
    let drop = null;
    const crisisSpec = S.enemies.find(function (e) { return e.crisis; });
    const eliteSpec = S.enemies.find(function (e) { return e.elite; });
    if (crisisSpec || (eliteSpec && Math.random() < 0.5)) drop = G.grantItem();
    G.save();

    const ov = G.el('div', 'victory-ov');
    const box = G.el('div', 'victory-box panel');
    box.appendChild(G.el('div', 'v-title', crisisSpec ? '🏆 CRISE CONTROLADA' : '✅ TAREFA CONCLUÍDA'));
    if (S.defeatLine) {
    ...
    stats.innerHTML =
      '<div>💵 +US$ ' + coins + ' de bônus</div>' +
      '<div>⭐ +' + xp + ' XP' + (lvls ? ' — <b>PROMOÇÃO: ' + G.DATA.rank(G.state.lvl).toUpperCase() + '!</b>' : '') + '</div>' +
      '<div>🎯 ' + acc + '% de acerto' + (acc >= 90 ? ' — bônus de excelência!' : '') + '</div>' +
      '<div>📈 Melhor uptime: x' + S.bestCombo + '</div>' +
      (drop ? '<div>📦 Do almoxarifado: ' + G.DATA.ITEMS[drop].e + ' ' + G.DATA.ITEMS[drop].name + '</div>' : '');
    box.appendChild(stats);
    const btn = G.el('button', 'btn primary wide', 'Continuar ▸');
    btn.onclick = function () {
      ov.remove();
      const cb = S.cfg.onEnd;
      const result = { win: true, boss: !!crisisSpec };
      B.stop();
      if (cb) cb(result);
    };
    box.appendChild(btn);
    ...
    if (crisisSpec) G.FX.confetti();
    if (lvls) G.AU.sfx('levelup');
  }

  function onPlayerDown() {
    if (G.state.inv.redundante > 0) {
      G.state.inv.redundante--;
      S.hp = Math.round(S.hpMax / 2);
      updateHud();
      renderItems();
      G.FX.flash('#57c7ff');
      G.AU.sfx('letter');
      G.toast('🛟 Sistema Redundante assumiu! Integridade restaurada em ' + S.hp + '.');
      G.save();
      return;
    }
    S.over = true;
    ...
    box.appendChild(G.el('div', 'v-title', '🚨 INCIDENTE REPORTADO'));
    box.appendChild(G.el('p', 'v-quote', 'Operação suspensa para investigação. Custo do retrabalho: US$ ' + lost + '. O que você aprendeu fica no relatório — e na memória.'));
    const btn = G.el('button', 'btn primary wide', 'Voltar ao ativo');
    btn.onclick = function () {
      ov.remove();
      const cb = S.cfg.onEnd;
      B.stop();
  ...
  function updateHud() {
    ...
    if (fixedMode()) { wrap.style.display = 'none'; }
      ...
      G.$('#b-php-label').textContent = '🛡 INTEGRIDADE · ' + (G.state.name || 'Você') + ' · ' + G.DATA.rank(G.state.lvl) + ' · ' + S.hp + '/' + S.hpMax;
    }
    const cb = G.$('#b-combo');
    if (S.combo >= 2) {
  ...
  function renderItems() {
    ...
    G.DATA.ITEM_ORDER.forEach(function (id) {
      ...
      if (id === 'redundante') { b.classList.add('passive'); b.disabled = true; }
  ...
  function useItem(id) {
    ...
    const ch = S.ch;
    if (id === 'cafe' || id === 'marmita') {
      const heal = id === 'cafe' ? 35 : 18;
      if (S.hp >= S.hpMax) { G.toast('Integridade já está em 100%.'); return; }
      S.hp = Math.min(S.hpMax, S.hp + heal);
      G.FX.floatText(window.innerWidth / 2, window.innerHeight - 130, '+' + heal + ' 🛡', 'heal');
    } else if (id === 'dicionario') {
      if (S.hintUsed) { G.toast('Uma consulta por pergunta. Norma da casa.'); return; }
      if (ch.ui === 'choice') {
        const btns = G.$$('.opt', G.$('#b-area')).filter(function (b) { return !b.disabled; });
        if (btns.length <= 2) { G.toast('Só restam duas alternativas — decida.'); return; }
        let removed = 0;
        ...
        if (!removed) { G.toast('O dicionário não ajudou desta vez.'); return; }
      ...
      } else { G.toast('A consulta não se aplica aqui.'); return; }
      S.hintUsed = true;
    } else if (id === 'impacto') {
      if (S.armCrit) { G.toast('A chave de impacto já está em punho.'); return; }
      S.armCrit = true;
      G.toast('🔧 Chave de Impacto em punho: próximo acerto vale DOBRO!');
    } else if (id === 'prorrogacao') {
      S.qStart += 8000;
      if (S.qStart > performance.now()) S.qStart = performance.now();
      G.toast('⏳ +8 segundos aprovados.');
    } else return;
    G.state.inv[id]--;
    G.AU.sfx('item');
    G.save();
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    if (flee) flee.onclick = function () {
      if (!S) return;
      G.modal({
        title: 'Suspender a tarefa?',
        html: 'Sem penalidade — mas a pendência continua no caminho da certificação.',
        buttons: [
          { label: 'Continuar executando', cls: 'primary' },
          {
            label: '⏸ Suspender', cls: 'ghost', cb: function () {
