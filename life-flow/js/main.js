/* LIFE FLOW — main.js
   ...
   Trilha única (World Class 1) — sem seletor de trilha. */
(function (root) {
  ...
  function renderSplash() {
    ...
    const btn = DF.$('#t-play');
    btn.innerHTML = hasSave ? '🏘️ CONTINUAR NO BAIRRO<span class="t-sub">' + DF.esc(DF.state.name) + '</span>' : '🏘️ ENTRAR NO BAIRRO';
    btn.onclick = function () {
      if (DF.AU) DF.AU.sfx('click');
      if (DF.state.name) goMap(); else DF.go('s-name');
    };
  ...
  function goMap() {
    DF.go('s-map');
    DF.STR.refresh();
    const focus = DF.STR.focusUnit();
    if (focus != null) setTimeout(function () { DF.STR.scrollToUnit(focus); }, 60);
  }
  DF.goMap = goMap;
  ...
  function bindName() {
    ...
    go.onclick = function () {
      ...
      DF.modal({
        title: '🏘️ Bem-vindo(a) ao bairro, ' + DF.esc(DF.state.name) + '!',
        html: 'Cada casa do bairro é uma unidade de <b>World Class 1</b>. Ande pela rua, unidade por unidade, até completar as 12 casas.<br><br>' +
          'As casas acendem quando seu mentor envia o <b>código de desbloqueio (UNL…)</b> pelo WhatsApp — cole em <b>🔑 Códigos</b>.',
        buttons: [{ label: 'Ver o bairro', cls: 'primary', cb: goMap }]
      });
    };
  ...
  function renderCodes() {
    ...
    c.appendChild(DF.el('div', 'screen-head',
      '<button class="btn ghost small" onclick="DF.go(\'s-map\')">← Bairro</button><h2>🔑 Códigos</h2>'));
    ...
    btn.onclick = async function () {
      ...
      try {
        ...
        if (r.type === 'unl') {
          ...
          DF.modal({
            title: '🔓 Casa desbloqueada!',
            html: (r.news.length ? 'Novas casas acesas no bairro:' : 'Casas já estavam acesas:') + '<br><br><b>' + names + '</b><br><br>— liberado por ' + DF.esc(r.mn),
            buttons: [{
              label: '🏘️ Ir até lá', cls: 'primary', cb: function () {
                goMap();
                if (r.units.length) setTimeout(function () { DF.STR.scrollToUnit(r.units[0]); }, 80);
              }
            }]
          });
          DF.STR.refresh();
          ...
          const kindName = t.t === 'blk' ? '🚨 REVIEW DO BLOCO' : t.t === 'fin' ? '🏆 FINAL TEST' : '📋 Teste de competência';
          DF.modal({
            title: kindName,
    ...
    bs.onclick = async function () {
      ...
      DF.modal({
        ...
        html: DF.codeBox(code, {
          wa: '🏘️ LIFE FLOW · backup de progresso de ' + (DF.state.name || '—') + ' (' + DF.fmtDate() + '):\n\n' + code,
  ...
  function renderSettings() {
    ...
    c.innerHTML = '';
    c.appendChild(DF.el('div', 'screen-head',
      '<button class="btn ghost small" onclick="DF.go(\'s-map\')">← Bairro</button><h2>⚙️ Ajustes</h2>'));
    ...
    if (DF.SP.ttsAvailable()) {
      ...
      sel.onchange = function () {
        ...
        DF.save();
        DF.SP.speak('Welcome to Life Flow.');
      };
      p.appendChild(sel);
    ...
    c.appendChild(DF.el('p', 'muted center foot-note',
      'LIFE FLOW · EnglishFlow<br>v' + DF.VERSION + ' · 100% offline · mentor via WhatsApp'));
    DF.go('s-settings');
  }
  ...
  function bindHud() {
    ...
    const map = DF.$('#s-map');
    map.addEventListener('scroll', DF.STR.onScroll, { passive: true });
  }
