/* LIFE FLOW — teacher.js
   Painel do mentor (roda local, sem servidor).
   Gera UNL (desbloqueio), TST (teste/Review/Final) e certificados PDF;
   lê e valida REL (assinatura anti-fraude) e mantém o ranking local. */
(function (root) {
  const DF = root.DF = root.DF || {};
  const KEY = 'lifeflow_teacher';
  ...
  function renderUnlock(panel) {
    ...
    p.appendChild(DF.el('p', 'muted', 'Escolha a(s) casa(s) para acender no mapa do aluno — em qualquer ordem, no seu ritmo.'));
    mentorField(p);
    ...
    go.onclick = async function () {
      ...
      showCode('🔓 Código de desbloqueio',
        code,
        '🏘️ LIFE FLOW — nova casa liberada!\n' + names +
  ...
  function renderTest(panel) {
    ...
    go.onclick = async function () {
      ...
      showCode('📋 Ordem de teste — Unit ' + u,
        code,
        '🏘️ LIFE FLOW — Teste de competência: Unit ' + u + ' — ' + DF.UNITS[u].name +
  ...
  // ---------------- aba Review / Final Test ----------------
  function renderExams(panel) {
    const p = DF.el('div', 'panel');
    p.appendChild(DF.el('p', 'muted', 'Reviews nativos do livro (blocos de 3 unidades) e o Final Test do bairro. Emita quando o bloco estiver concluído nas aulas ao vivo.'));
    mentorField(p);
    const sn = studentField(p);
    ...
    go.onclick = async function () {
      ...
      const code = await DF.CODES.makeTest({
        t: cp.id === 'fin' ? 'fin' : 'blk',
        us: cp.units, cats: ['voc', 'gra', 'num', 'lis', 'rea', 'fun'],
  ...
  function renderReports(panel) {
    ...
    btn.onclick = async function () {
      ...
      try {
        ...
        const kindName = d.k === 'blk' ? '🚨 Review' : d.k === 'fin' ? '🏆 Final Test' :
          d.k === 'unit' ? '🏘️ Relatório de unidade' : '📋 Teste';
        card.innerHTML =
        ...
        Object.keys(d.cats || {}).forEach(function (k) {
          ...
          const info = { voc: '📦 Vocabulário', gra: '🔧 Gramática', num: '🔢 Números', lis: '🎧 Listening', rea: '📖 Leitura', fun: '🧰 Frases úteis' };
  ...
  const CERT_LEVELS = [
    ...
    { id: 'block', label: '🚨 Review aprovado', kicker: 'CERTIFICADO DE APROVAÇÃO', body: 'foi aprovado(a) no Review de' },
    { id: 'trail', label: '🏆 Bairro completo', kicker: 'CERTIFICADO DE CONCLUSÃO', body: 'concluiu integralmente' }
  ];
  ...
  function renderCerts(panel) {
    ...
    inpDetail.placeholder = 'ex.: Unit 5 — No Need to Panic · ou · World Class 1';
    ...
    go.onclick = function () {
      ...
      const detail = inpDetail.value.trim() || 'World Class 1';
  ...
  function drawCert(lvl, name, detail, mn) {
    const html =
      ...
      '<div class="cert-kicker">EnglishFlow · Life Flow</div>' +
      '<div class="cert-title">' + lvl.kicker + '</div>' +
      '<div class="cert-body">Certificamos que<br>' +
      '<span class="cert-name">' + DF.esc(name) + '</span>' +
      lvl.body + ' <b>' + DF.esc(detail) + '</b>, demonstrando domínio do inglês avaliado.' +
      '<div class="cert-detail">World Class 1 · National Geographic Learning</div></div>' +
      '<div class="cert-foot">' +
