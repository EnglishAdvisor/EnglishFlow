/* BUSINESS FLOW — missions.js
   Tela do andar (unidade), corredor de missões, execução de missão,
...
(function (root) {
  ...
  function resolveItems(u, def, sec) {
    ...
    if (!items.length) {
      ...
      items.push({
        ...
        title: '🚧 ' + sec.name,
        text: 'Esta missão está em produção e chega numa próxima fase do BUSINESS FLOW.\n\nEnquanto isso, revise o vocabulário do andar abaixo.',
  ...
  MI.renderUnit = function (u) {
    ...
    head.appendChild(DF.el('div', 'unit-title',
      '<span class="u-chip" style="background:' + meta.ac + '22;color:' + meta.ac + ';border-color:' + meta.ac + '55">UNIT ' + u + ' · ' + meta.depth + 'º andar</span>' +
    ...
    if (prog.done >= prog.total && prog.total > 0) {
      relBox.appendChild(DF.el('p', '', '🏁 <b>Andar concluído!</b> Gere seu relatório assinado e envie ao mentor:'));
      ...
      relBox.appendChild(DF.el('p', 'muted', '📜 Conclua todas as missões para gerar o relatório do andar (REL) e enviá-lo ao mentor.'));
    }
  ...
  function finishMission() {
    ...
    const done = DF.el('button', 'btn primary wide', 'Voltar ao andar ▸');
  ...
  MI.startReview = function () {
    ...
    if (!items.length) {
      DF.modal({
        title: '🛠️ Manutenção',
        html: 'Nenhuma NC pendente. Tudo conforme! ✅<br><br>Erre menos, suba mais. 🏢',
  ...
  MI.unitReport = async function (u) {
    ...
    const meta = DF.UNITS[u];
    const wa = '🏢 BUSINESS FLOW · Relatório da Unit ' + u + ' — ' + meta.name +
