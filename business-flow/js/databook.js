/* BUSINESS FLOW — databook.js
   Data Book do aluno: domínio por competência, progresso por andar,
...
(function (root) {
  ...
  DB.render = function () {
    ...
    pu.appendChild(DF.el('h3', '', '🏢 Progresso da subida'));
    Object.keys(DF.UNITS).forEach(function (k) {
      ...
      row.innerHTML = '<span class="cat-lbl">' + u.icon + ' U' + u.n + ' · ' + 'andar ' + u.depth + '</span>' +
        bar(prog.pct, u.ac) + '<span class="cat-pct">' + prog.done + '/' + prog.total + '</span>';
      pu.appendChild(row);
    });
    if (!Object.keys(DF.state.unlocked).length) {
      pu.appendChild(DF.el('p', 'muted', 'Nenhum andar desbloqueado ainda.'));
    }
    c.appendChild(pu);
    ...
    DF.state.tests.slice().reverse().forEach(function (t) {
      ...
      const kindName = t.k === 'blk' ? '🚨 Viewpoint' : t.k === 'fin' ? '🏆 Final Test' : '📋 Unit ' + t.us.join(',');
      ...
      re.onclick = function () {
        DF.modal({
          ...
          html: DF.codeBox(t.code, {
            wa: '🏢 BUSINESS FLOW · comprovação ' + kindName + ' — ' + t.sc + '/1000\n\n' + t.code,
