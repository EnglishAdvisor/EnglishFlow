/* DEEP FLOW — databook.js
   Data Book do aluno: domínio por competência, progresso por estação,
...
(function (root) {
  ...
  DB.render = function () {
    ...
    // progresso por estação
    const pu = DF.el('div', 'panel');
    pu.appendChild(DF.el('h3', '', '🌊 Progresso da descida'));
    Object.keys(DF.UNITS).forEach(function (k) {
      ...
      row.innerHTML = '<span class="cat-lbl">' + u.icon + ' U' + u.n + ' · ' + u.depth + 'm</span>' +
        bar(prog.pct, u.ac) + '<span class="cat-pct">' + prog.done + '/' + prog.total + '</span>';
      pu.appendChild(row);
    });
    if (!Object.keys(DF.state.unlocked).length) {
      pu.appendChild(DF.el('p', 'muted', 'Nenhuma estação desbloqueada ainda.'));
    }
    c.appendChild(pu);
    ...
    DF.state.tests.slice().reverse().forEach(function (t) {
      ...
      const kindName = t.k === 'blk' ? '🚨 Block Exam' : t.k === 'fin' ? '🏆 Final Test' : '📋 Unit ' + t.us.join(',');
      ...
      re.onclick = function () {
        DF.modal({
          ...
          html: DF.codeBox(t.code, {
            wa: '🌊 DEEP FLOW · comprovação ' + kindName + ' — ' + t.sc + '/1000\n\n' + t.code,
