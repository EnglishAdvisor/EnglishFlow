/* A Maldição da Cuca — mentor.js
   Conexão mentor ↔ aluno sem servidor: códigos compactados (deflate + base64url)
   que viajam por WhatsApp. Resultados e boletins são assinados — o app do mentor
   verifica a assinatura antes de aceitar. */
(function (root) {
  ...
  M.makeResult = function (payload) {
    ...
    return pack(payload, 'PROVA1');
  };
  M.verifyResult = async function (code) {
    const r = await unpack(code, 'PROVA1');
    const ok = r.sig === G.sign(canon(r));
    return { verified: ok, data: r };
  };
  ...
  //                         TELAS
  // ============================================================
  const MISSION_KINDS = ['venpt', 'vpten', 'ff', 'odd', 'build', 'ltype', 'lpair', 'ed', 'irr', 'prep', 'phg', 'phm'];

  M.renderMentor = function () {
    const c = G.$('#s-mentor .content');
    c.innerHTML = '';
    c.appendChild(G.el('div', 'screen-head',
      '<button class="btn ghost small" onclick="G.go(\'s-map\')">← Voltar</button><h2>🎓 Modo Mentor</h2>'));
    const tabs = G.el('div', 'tabs');
    const panels = G.el('div', '');
    const defs = [
      { id: 'criar', label: '✏️ Criar Missão', render: renderCreate },
      { id: 'result', label: '🏆 Resultados', render: renderResults },
      { id: 'boletim', label: '📊 Boletins', render: renderReports }
    ];
  ...
  function renderCreate(panel) {
    ...
    p.appendChild(G.el('label', 'lbl', 'Seu nome (aparece para o aluno)'));
    const inName = G.el('input', 'inp'); inName.maxLength = 24; inName.value = st.mentorName || '';
    inName.placeholder = 'Prof. Maria'; p.appendChild(inName);

    p.appendChild(G.el('label', 'lbl', 'Título da missão'));
    const inTitle = G.el('input', 'inp'); inTitle.maxLength = 40; inTitle.placeholder = 'Revisão da semana 3';
    p.appendChild(inTitle);

    p.appendChild(G.el('label', 'lbl', 'Recado para o aluno (opcional)'));
    const inMsg = G.el('input', 'inp'); inMsg.maxLength = 140; inMsg.placeholder = 'Capriche! Vale ponto extra.';
    p.appendChild(inMsg);
    ...
    col2.appendChild(G.el('label', 'lbl', 'Dificuldade'));
    const selD = G.el('select', 'inp');
    [['1', 'Fácil'], ['2', 'Média'], ['3', 'Difícil']].forEach(function (d) { selD.appendChild(new Option(d[1], d[0])); });
    col2.appendChild(selD);
    row.appendChild(col1); row.appendChild(col2);
    p.appendChild(row);

    p.appendChild(G.el('label', 'lbl', 'Tipos de desafio'));
    ...
    checks.vpten.classList.add('on');
    p.appendChild(kwrap);

    p.appendChild(G.el('label', 'lbl', '📚 Vocabulário próprio (opcional) — uma linha por palavra: english = português'));
    const taV = G.el('textarea', 'inp'); taV.rows = 4; taV.placeholder = 'butterfly = borboleta\nlighthouse = farol';
    p.appendChild(taV);

    p.appendChild(G.el('label', 'lbl', '✍️ Frases próprias (opcional) — English sentence = tradução'));
    const taS = G.el('textarea', 'inp'); taS.rows = 3; taS.placeholder = 'The early bird catches the worm = Deus ajuda quem cedo madruga';
    p.appendChild(taS);

    const go = G.el('button', 'btn primary wide', '🪄 Gerar código da missão');
    go.onclick = async function () {
      const kinds = MISSION_KINDS.filter(function (k) { return checks[k].classList.contains('on'); });
      if (!kinds.length) { G.toast('Escolha ao menos um tipo de desafio!'); return; }
      st.mentorName = inName.value.trim() || 'Mentor';
      G.save();
      const m = {
        v: 1, id: Math.random().toString(36).slice(2, 8),
        mn: st.mentorName, t: inTitle.value.trim() || 'Missão de Inglês',
      ...
      go.disabled = false; go.textContent = '🪄 Gerar código da missão';
      if (G.AU) G.AU.sfx('letter');
      const wa = '🐊 Missão de inglês: "' + m.t + '" — de ' + m.mn +
        '\nAbra o jogo A Maldição da Cuca, toque em MISSÕES e cole o código:\n\n' + code;
      G.modal({
        title: '📨 Missão pronta!',
        html: (function () {
          const d = G.el('div');
          d.appendChild(G.el('p', 'muted', 'Envie este código para seus alunos. Quando terminarem, eles te mandam o código de comprovação — confira na aba Resultados.'));
  ...
  function renderResults(panel) {
    const p = G.el('div', 'panel');
    p.appendChild(G.el('label', 'lbl', 'Cole aqui o código de comprovação do aluno (PROVA1...)'));
    const ta = G.el('textarea', 'inp'); ta.rows = 4; ta.placeholder = 'PROVA1.C.xxxxx...';
    p.appendChild(ta);
    const btn = G.el('button', 'btn primary wide', '🔎 Verificar resultado');
    const out = G.el('div', '');
    btn.onclick = async function () {
      ...
      try {
        ...
        const badge = r.verified
          ...
          : '<span class="badge bad">⚠ CÓDIGO ADULTERADO</span>';
        card.innerHTML =
        ...
        G.$('.mission-line', card).textContent = (d.mt || 'Missão') + ' · ' + (d.d || '');
        ...
        if (r.verified) {
          const save = G.el('button', 'btn ghost', '💾 Salvar no ranking');
        ...
        out.appendChild(G.el('p', 'warn', '❌ Não entendi esse código. Confira se copiou tudo (começa com PROVA1).'));
      }
    };
    p.appendChild(btn);
    p.appendChild(out);
    ...
    rp.appendChild(G.el('h3', '', '🏆 Ranking da turma'));
    const list = G.el('div', '');
    rp.appendChild(list);
    function drawRoster() {
      list.innerHTML = '';
      ...
      if (!roster.length) {
        list.appendChild(G.el('p', 'muted', 'Nenhum resultado salvo ainda.'));
        return;
      }
      ...
      clear.onclick = function () {
        G.modal({
          title: 'Limpar ranking?', html: 'Todos os resultados salvos serão apagados.',
          buttons: [
            { label: 'Cancelar', cls: 'ghost' },
            { label: 'Apagar', cls: 'danger', cb: function () { G.state.roster = []; G.save(); drawRoster(); } }
          ]
        });
      };
  ...
  function renderReports(panel) {
    const p = G.el('div', 'panel');
    p.appendChild(G.el('p', 'muted', 'O aluno gera o boletim em Ajustes → "Boletim de progresso" e te envia. Aqui você vê os pontos fortes e fracos dele.'));
    p.appendChild(G.el('label', 'lbl', 'Cole o boletim (BOLETIM1...)'));
    const ta = G.el('textarea', 'inp'); ta.rows = 4; ta.placeholder = 'BOLETIM1.C.xxxxx...';
    p.appendChild(ta);
    const btn = G.el('button', 'btn primary wide', '📖 Ler boletim');
    const out = G.el('div', '');
    btn.onclick = async function () {
      out.innerHTML = '';
      try {
        ...
        G.$('.head-line', card).textContent =
          'Nível ' + (d.lvl || 1) + ' · ' + (d.bat || 0) + ' batalhas · ' + (d.boss || 0) + ' lendas · ' +
          (d.mast || 0) + '/' + (d.seen || 0) + ' palavras dominadas · ' + (d.d || '');
        ...
        if (weakest && weakest.pct < 70) {
          cats.appendChild(G.el('p', 'warn', '💡 Sugestão: reforçar "' + weakest.label + '" (' + weakest.pct + '%). Crie uma missão só desse tipo!'));
        }
        out.appendChild(card);
        if (G.AU) G.AU.sfx(r.verified ? 'good' : 'bad');
      } catch (e) {
        out.appendChild(G.el('p', 'warn', '❌ Não entendi esse código. Confira se copiou tudo (começa com BOLETIM1).'));
      }
    };
    p.appendChild(btn);
    p.appendChild(out);
  ...
  // ---------------- tela do aluno: missões ----------------
  M.renderMissions = function () {
    const c = G.$('#s-missions .content');
    c.innerHTML = '';
    c.appendChild(G.el('div', 'screen-head',
      '<button class="btn ghost small" onclick="G.go(\'s-map\')">← Voltar</button><h2>✉️ Missões do Mentor</h2>'));

    const p = G.el('div', 'panel');
    p.appendChild(G.el('label', 'lbl', 'Recebeu um código do seu professor ou mentor? Cole aqui:'));
    const ta = G.el('textarea', 'inp'); ta.rows = 4; ta.placeholder = 'CUCA1.C.xxxxx...';
    p.appendChild(ta);
    const btn = G.el('button', 'btn primary wide', '📬 Carregar missão');
    ...
    btn.onclick = async function () {
      ...
      try {
        ...
        G.$('.mfrom', card).textContent = 'De: ' + m.mn + ' · ' + m.n + ' perguntas · ' +
          ['', 'Fácil', 'Média', 'Difícil'][m.diff];
        ...
        const start = G.el('button', 'btn primary wide', '⚔️ Começar missão');
        start.onclick = function () { G.startMission(m); };
        card.appendChild(start);
        preview.appendChild(card);
        if (G.AU) G.AU.sfx('good');
      } catch (e) {
        preview.appendChild(G.el('p', 'warn', '❌ Código inválido. Confira se copiou tudo (começa com CUCA1).'));
    ...
    if (hist.length) {
      ...
      hp.appendChild(G.el('h3', '', '📚 Missões concluídas'));
  ...
  function waResultText(t, sc, ac, code) {
    return '🐊 Missão "' + t + '" concluída! ' + sc + '/1000 (' + ac + '% de acerto)\nCódigo de comprovação:\n\n' + code;
  }

  // chamada pela batalha ao terminar uma missão
  M.finishMission = async function (mission, stats) {
    const st = G.state;
    const payload = {
      v: 1, mid: mission.id, mt: mission.t,
      st: st.name || 'Estudante',
    ...
    d.appendChild(G.el('p', 'muted', 'Envie o código abaixo para ' + mission.mn + ' comprovar que você completou a missão:'));
